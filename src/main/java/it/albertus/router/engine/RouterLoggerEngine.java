...
public abstract class RouterLoggerEngine {
	...
	public synchronized void scheduleShutdown(final int timeoutInSecs) {
		if (shutdownDaemon != null) {
			shutdownDaemon.interrupt();
		}
		if (timeoutInSecs >= 0) {
			shutdownDaemon = new ShutdownDaemon(this, timeoutInSecs);
	...
	public ShutdownDaemon getShutdownDaemon() {
